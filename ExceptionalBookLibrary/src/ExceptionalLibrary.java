...
public class ExceptionalLibrary {
    ...
    public int parsePinCode(String s, int errorOffset) throws ParseException {
        int pinMax = 9999; // highest number a pin can be
        int pinMin = 1000; // lowest number a pin can be
        int intPin;// pin code as an int
        try {
            intPin = Integer.parseInt(s); // parse the String argument s as a pin code
        } catch (NumberFormatException e) { // syntax error
            throw new ParseException("Error: The pin code MUST be a NUMBER.\n", errorOffset);
        }
        if(intPin > pinMax || intPin < pinMin){ // check if intPin is within acceptable number range
            throw new ParseException("Error: the pin code must be between 1000 and 9999.\n",errorOffset);
        }
        return intPin;
    }
    ...
    public int parseCardBarCode(String s, int errorOffset) throws ParseException {
        int intBarCode; // bar code as an int
        try {
            intBarCode = Integer.parseInt(s); // parse the String argument s as a bar code
        } catch (NumberFormatException e) { // syntax error
            throw new ParseException("Error: The card bar code MUST be a NUMBER.\n", errorOffset);
        }
        if(!Subscriber.checkCardBarCode(intBarCode)){ // check if intBarCode is within max/min range
            throw new ParseException("Error: The card bar code MUST be a valid code.\n", errorOffset);
        }
        return intBarCode;
    }
     ...
     * @throws ParseException f the provided bookIdArgument cannot be parsed as an integer with
     * the following error message "Error: Book id MUST be an integer."
     */
    public int parseBookId(String s, int errorOffset) throws ParseException {
        int intID; // book ID as an int
        try {
            intID = Integer.parseInt(s); // parse the String argument s as a book ID
        } catch (NumberFormatException e) { // syntax error
            throw new ParseException("Error: The book ID MUST be a NUMBER.\n", errorOffset);
        }
        return intID;
    }
    ...
    public void parseRunLibrarianAddSubscriberCommand(String[] commands)
        ...
        checkCommandArgumentsCount(commands, 5); // length with respect to a provided count
        parsePinCode(commands[2],2); // check if pin code is valid
        parsePhoneNumber(commands[4],4); // check if phone number is valid
    ...
    public void parseRunLibrarianCheckoutBookCommand(String[] commands) throws ParseException {
        checkCommandArgumentsCount(commands, 3); // length with respect to a provided count
        parseCardBarCode(commands[1],1); // check if bar code is valid
        parseBookId(commands[2],2); // check if book Id is valid
    ...
    public void parseRunLibrarianDisplayPersonalInfoOfSubscriberCommand(String[] commands)
            throws ParseException {
        checkCommandArgumentsCount(commands,2); // length with respect to a provided count
        parseCardBarCode(commands[1],1); // check if bar code is valid
    ...
    public void parseRunLibrarianDisplayBooksCheckedOutBySubscriberCommand(String[] commands)
            throws ParseException {
        checkCommandArgumentsCount(commands,2); // length with respect to a provided count
        parseCardBarCode(commands[1],1);// checks if bar code is valid
    ...
    public void parseRunLibrarianRemoveBookCommand(String[] commands) throws ParseException {
        checkCommandArgumentsCount(commands,2); // length with respect to a provided count
        parseBookId(commands[1],1); // checks if book ID is valid
    ...
    public void parseRunSubscriberCheckoutBookCommand(String[] commands, Subscriber subscriber)
        ...
        checkCommandArgumentsCount(commands,2); // length with respect to a provided count
        parseBookId(commands[1],1); // checks if book ID is valid
        // look for the book and checks it out if it is not already checked out by the subscriber
    ...
    public void parseRunSubscriberUpdatePhoneNumberCommand(String[] commands, Subscriber subscriber)
            throws ParseException {
        checkCommandArgumentsCount(commands,2); // length with respect to a provided count
        parsePhoneNumber(commands[1],1); // checks if phone number is valid
    ...
    public void parseRunSubscriberFindBooksByTitleCommand(String[] commands, Subscriber subscriber)
        ...
        this.checkCommandArgumentsCount(commands, 2); //length with respect to a provided count
    ...
    public void parseRunLibrarianSaveBooksCommand(String[] commands) throws ParseException {
        checkCommandArgumentsCount(commands,2); // length with respect to a provided count
        File bookLoad = new File(commands[1]); // creates new file based on user input
        PrintWriter writer = null;
        try {
            writer = new PrintWriter(bookLoad);
            // Adds each element of books into a file using PrintWriter.
            for (Book saveBooks : books) {
                String saveTitle = saveBooks.getTitle();
                String saveAuthor = saveBooks.getAuthor();
                writer.println(saveTitle + ":" + saveAuthor); // formats input correctly
            }
        }catch (FileNotFoundException e){ // if file doesn't exist, print out error
            System.out.println("Error reading: " + commands[1]);
        } finally { // closes printwriter
            writer.close();
        }
    }
    ...
    public void parseRunLibrarianLoadBooksCommand(String[] commands) throws ParseException {
        checkCommandArgumentsCount(commands, 2); // length with respect to a provided count
        ArrayList<String> fileLines = new ArrayList<>(); // arraylist to store contents of file
        File bookFile = new File(commands[1]); // creates new file based on uer input
        Scanner scnr = null; // scanner to read file
        try {
            scnr = new Scanner(bookFile);
            ArrayList<String> titleAuthorList; // stores titles and authors of books
            while (scnr.hasNextLine()) { // continues to read file until there are no more lines
            ...
            for (int i = 0; i < fileLines.size(); i++) {
                ...
                // ignores single semicolons
                if (fileLines.get(i).trim().length() < 3) {
                    continue;
                }
                String books = fileLines.get(i).trim(); // formats each line of files
                String[] titleAuthor = books.split(":"); // splits titles and authors
                titleAuthorList = new ArrayList<>(Arrays.asList(titleAuthor));
                // goes through each title and author
                for (int j = 0; j < titleAuthorList.size(); j += 2) {
                    // break at very last element
                    if (j == titleAuthorList.size() - 1) {
                        break;
                    }
                    String title = titleAuthorList.get(j).trim();
                    String author = titleAuthorList.get(j + 1).trim();
                    addBook(title, author); // adds books into library
                }
            }
        }catch(FileNotFoundException e){ // if file does not exist, print out error
            System.out.println("Error reading: " + commands[1]);
        } finally {
            scnr.close(); // closes scanner that reads file
        }
    }
    ...
    public void parseRunLoginAsSubscriber(String[] commands, Scanner scanner) throws ParseException {
        parseCardBarCode(commands[1],1); //length with respect to a provided count
        checkCommandArgumentsCount(commands,3);
        ...
        while (!(commands[0].toUpperCase().equals("9") && commands.length == 1)) { // "9": Exit the
            ...
            switch (commands[0].toUpperCase()) {
                case "1": // add a new book commands[1] title; commands[2] author
                    ...
                    } catch(ParseException e){ // print out error is method runs incorrectly
                ...
                case "2": // add a new subscriber: commands[1] name; commands[2] pin; commands[3] address;
                    ...
                    catch(InstantiationException e){ // print out error is method runs incorrectly
                        System.out.println(e);
                    }
                    break;
            ...
            // read and split next user command line
            displayLibrarianMenu(); // display the librarian management system  menu
            System.out.print(promptCommandLine);
            command = scanner.nextLine(); // read user command line
            commands = command.trim().split(" "); // split user command line
        }
    ...
    public void readProcessSubscriberCommand(Subscriber subscriber, Scanner scanner) {
        ...
        while (!(commands[0].toUpperCase().equals("9") && commands.length == 1)) { // "9": Exit the
            ...
            // read and split next user command line
            displaySubscriberMenu(); // display the subscriber management system menu
            System.out.print(promptCommandLine);
            command = scanner.nextLine(); // read user command line
            commands = command.trim().split(" "); // split user command line
        }
